...
#   For this to work correctly, all values D[x] stored in the dictionary must be comparable.
# - Second, iterating "for x in D" finds and removes the items from D in sorted order.
#   Each item is not removed until the next item is requested, so D[x] will still return
#   a useful value until the next iteration of the for-loop.
